...
type CallbackInput struct {
	...
	Responses []*schema.AgenticResponse
...
type CallbackOutput struct {
	...
	Response *schema.AgenticResponse
	// Config is the config for the model.
	Config *Config
...
func ConvCallbackInput(src callbacks.CallbackInput) *CallbackInput {
	...
	case *CallbackInput: // when callback is triggered within component implementation, the input is usually already a typed *model.CallbackInput
		return t
	case []*schema.AgenticResponse: // when callback is injected by graph node, not the component implementation itself, the input is the input of Chat Model interface, which is []*schema.Message
...
func ConvCallbackOutput(src callbacks.CallbackOutput) *CallbackOutput {
	...
	case *CallbackOutput: // when callback is triggered within component implementation, the output is usually already a typed *model.CallbackOutput
		return t
	case *schema.AgenticResponse: // when callback is injected by graph node, not the component implementation itself, the output is the output of Chat Model interface, which is *schema.Message
